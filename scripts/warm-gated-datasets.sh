...
#
#   ./slurm/with-hf-token.sh -- ./scripts/warm-gated-datasets.sh HOST [HOST...]
#
...
# serves it and slurm/with-hf-token.sh can go on refusing to hand tokens to
# sbatch.
...
if [[ $# -lt 1 ]]; then
    ...
    echo "  run under slurm/with-hf-token.sh so HF_TOKEN is set" >&2
    exit 64
fi
if [[ -z "${HF_TOKEN:-}" && "$DRY_RUN" -eq 0 ]]; then
    echo "error: HF_TOKEN is not set; run this under slurm/with-hf-token.sh" >&2
    exit 64
fi
