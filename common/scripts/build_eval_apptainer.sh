...
DEFINITION="$PROJECT_DIR/common/apptainer/eval-vllm.def"

usage() {
    cat <<'EOF'
Usage: common/scripts/build_eval_apptainer.sh BASE_IMAGE OUTPUT.sif

BASE_IMAGE must be an immutable docker:// reference ending in @sha256:<64 hex>.
Example:
  common/scripts/build_eval_apptainer.sh \
