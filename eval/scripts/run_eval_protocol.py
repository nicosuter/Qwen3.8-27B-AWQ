...
# The checkpoint's MTP artifact is described on the quantization side, and this
# is the eval side reading that description back. One directional dependency,
# never the reverse.
...
try:
    ...
    # repository root, rather than eval/scripts/, on sys.path.
    from eval.scripts import eval_suite  # type: ignore[no-redef]
...
def audit_overlap(
    ...
    command = [
        sys.executable,
        str(PROJECT_DIR / "eval" / "scripts" / "audit_eval_overlap.py"),
        "--calibration",
        str(calibration),
    ]
    prompt_to_suite = {}
...
def check_compatibility(
    ...
    command = prefix + [
        "python3",
        str(PROJECT_DIR / "eval" / "scripts" / "check_eval_compat.py"),
        "--baseline",
        baseline["model"],
        "--baseline-revision",
        baseline["revision"],
...
def run_mtp(config: dict[str, Any], image: Path, run_dir: Path) -> dict[str, int]:
    ...
    for concurrency in mtp["concurrencies"]:
        ...
        command = [
            sys.executable,
            str(PROJECT_DIR / "eval" / "scripts" / "compare_mtp_results.py"),
            "--disabled",
...
def compare_results(
    ...
    command = [
        sys.executable,
        str(PROJECT_DIR / "eval" / "scripts" / "compare_eval_results.py"),
        "--baseline",
        str(baseline),
        "--candidate",
        str(candidate),
