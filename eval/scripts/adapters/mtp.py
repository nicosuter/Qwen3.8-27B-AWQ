#!/usr/bin/env python3
"""MTP gate adapter for eval/scripts/run_eval_protocol.py.
...
`eval/scripts/compare_mtp_results.py` consumes.
...
# The speculative-decoding counters this adapter reads are parsed by the same
# module the quantization-side serve smoke uses, so it lives in common/.
COMMON_SCRIPTS = Path(__file__).resolve().parents[3] / "common" / "scripts"
