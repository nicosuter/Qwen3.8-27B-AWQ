...
# Every accept type the registry might answer with; without them it returns a v1
# manifest whose digest is not the one a puller would resolve.
MANIFEST_TYPES = (
...
def resolve_digest(image: str, timeout: float = 60.0) -> str:
    """Resolve `repo:tag` to the digest a pull would get right now.

    SWE-bench Pro's images are official -- scaleapi/SWE-bench_Pro-os names
    jefzda/sweap-images itself -- but every task references them by tag, and the
    upstream project documents no immutability guarantee. A tag is a moving
    pointer: the subset file pins which tasks run and their digests, and none of
    that covers the image the task actually executes in. Building from the digest
    makes the .sif reproducible and records what it was reproduced from.
    """
    ...
    if not tag:
        tag = "latest"
    with urllib.request.urlopen(AUTH.format(repo=repo), timeout=timeout) as response:
        token = json.load(response)["token"]
    request = urllib.request.Request(f"{REGISTRY}/v2/{repo}/manifests/{tag}", method="HEAD")
    request.add_header("Authorization", f"Bearer {token}")
    for accept in MANIFEST_TYPES:
        request.add_header("Accept", accept)
    with urllib.request.urlopen(request, timeout=timeout) as response:
        digest = response.headers.get("Docker-Content-Digest")
    if not digest or not digest.startswith("sha256:"):
        raise BakeError(f"registry returned no digest for {image}")
    return digest
...
def definition(parsed: dict[str, Any], ensure_tmux: bool = True,
               pinned_image: str | None = None) -> str:
    exports = "\n".join(f"    export {entry}" for entry in parsed["envs"])
    body = "\n".join(f"    {command}" for command in parsed["runs"])
    lines = [
        "Bootstrap: docker",
        f"From: {pinned_image or parsed['image']}",
        "",
        "%post",
        "    set -e",
...
def bake(task_dir: Path, sif_dir: Path, timeout: float,
         ensure_tmux: bool = True, pin_digest: bool = True) -> dict[str, Any]:
    ...
    # Resolved before the build so the .sif is built from the digest, not from
    # whatever the tag points at by the time apptainer pulls it.
    digest = resolve_digest(parsed["image"]) if pin_digest else None
    pinned_image = f"{parsed['image'].partition(':')[0]}@{digest}" if digest else None
    definition_path.write_text(
        definition(parsed, ensure_tmux, pinned_image), encoding="utf-8"
    )
    if not sif_path.is_file():
    ...
    return {
        "task": task_dir.name,
        "image": parsed["image"],
        # What the tag resolved to when this was baked. The upstream project
        # documents no immutability guarantee, so this is the only record of
        # which image the .sif actually contains.
        "image_digest": digest,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--no-pin-digest", action="store_true",
        help="build from the tag rather than resolving it to a digest first",
    )
    ...
    for index, task_dir in enumerate(tasks, start=1):
        try:
            record = bake(task_dir, args.sif_dir, args.timeout,
                          ...
                          pin_digest=not args.no_pin_digest)
